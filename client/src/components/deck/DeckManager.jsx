...
function DeckManager() {
  ...
  return (
    <div id="deck-manager-section" className="bg-white dark:bg-stone-800 rounded-lg shadow-xl p-6 lg:p-8 transition-colors">
      ...
      <div className="mb-8 p-4 rounded-lg bg-brand-50 dark:bg-brand-900/30 border border-brand-200 dark:border-brand-700">
        ...
        <form onSubmit={showYtFields ? handleImportPlaylistSubmit : handleShowImportFields} className="flex flex-col gap-3 items-stretch">
          <div className="flex flex-col sm:flex-row gap-3 items-center">
            <input
              type="url"
              className="flex-1 rounded-md border-stone-300 dark:border-stone-600 focus:border-brand-500 focus:ring-brand-500 bg-white dark:bg-stone-700 text-stone-900 dark:text-white px-3 py-2 transition-colors"
        ...
        {!showSplitUI ? (
          ...
          <form onSubmit={handleSplitVideoValidate} className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row gap-3 items-center">
              <input
                type="url"
                className="flex-1 rounded-md border-stone-300 dark:border-stone-600 focus:border-purple-500 focus:ring-purple-500 bg-white dark:bg-stone-700 text-stone-900 dark:text-white px-3 py-2 transition-colors"
          ...
          <div className="space-y-4">
            ...
            {splitVideoData?.videoId && (
              <div className="aspect-video w-full max-w-2xl mx-auto">
                <iframe
                  ...
                  allowFullScreen
                  className="w-full h-full rounded-lg border border-stone-200 dark:border-stone-700"
                />
              </div>
            )}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto">
                      {splitVideoData.chapters.map((chapter, index) => (
                        <button
                          ...
                          className="flex items-start gap-2 p-2 rounded-md bg-white dark:bg-stone-800 hover:bg-brand-100 dark:hover:bg-brand-900/30 transition-colors text-left border border-brand-200 dark:border-brand-700"
                        >
                          ...
                          <span className="text-sm text-stone-800 dark:text-stone-200 line-clamp-2">
                            {chapter.title}
                          </span>
                        </button>
                      ))}
            ...
            {/* Add Checkpoint Form */}
            <div className="bg-white dark:bg-stone-800 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
              <h5 className="font-medium text-stone-800 dark:text-stone-200 mb-3">Add Checkpoint</h5>
              <form onSubmit={handleAddCheckpoint} className="flex flex-col sm:flex-row gap-3">
                <input
                  type="text"
                  className="w-28 rounded-md border-stone-300 dark:border-stone-600 focus:border-purple-500 focus:ring-purple-500 bg-white dark:bg-stone-700 text-stone-900 dark:text-white px-3 py-2 text-center transition-colors"
                  placeholder="MM:SS"
            ...
            {checkpoints.length > 0 && (
              <div className="bg-white dark:bg-stone-800 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
                <h5 className="font-medium text-stone-800 dark:text-stone-200 mb-3">
            ...
            {/* Deck Configuration */}
            <div className="bg-white dark:bg-stone-800 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
              <h5 className="font-medium text-stone-800 dark:text-stone-200 mb-3">Deck Configuration</h5>
      ...
      <form onSubmit={handleSubmit} className="mb-8 space-y-4">
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full rounded-md border-stone-300 dark:border-stone-600 shadow-sm focus:border-brand-500 focus:ring-brand-500 bg-white dark:bg-stone-700 text-stone-900 dark:text-white transition-colors"
            required
          />
        </div>
        <div>
        ...
        <div className="flex items-center">
          <input
            ...
            className="h-4 w-4 text-brand-600 border-stone-300 dark:border-stone-600 rounded focus:ring-brand-500"
          />
          <label htmlFor="isPublic" className="ml-2 block text-sm text-stone-900 dark:text-stone-300">
            Publicly visible
          </label>
        </div>
        <div className="flex justify-end gap-4">
          {editingDeck && (
            <button
              ...
              className="px-4 py-2 rounded-md text-stone-700 dark:text-stone-200 bg-stone-100 dark:bg-stone-600 hover:bg-stone-200 dark:hover:bg-stone-500 transition-colors"
            >
              Cancel
            </button>
          )}
      ...
      <div className="border-t border-stone-200 dark:border-stone-700 pt-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold text-stone-700 dark:text-stone-200">Your Decks</h3>
        ...
        {isLoadingDecks ? (
          <p className="text-stone-500 dark:text-stone-400">Loading decks...</p>
          ...
          <ul className="space-y-3">
            {filteredDecks.map((deck) => (
              <li
                key={deck._id}
                className="p-4 rounded-md flex justify-between items-center transition-colors bg-stone-50 dark:bg-stone-700/50 hover:bg-stone-100 dark:hover:bg-stone-700 border border-stone-200 dark:border-stone-700"
              >
                <div>
                  <p className="font-semibold text-stone-800 dark:text-stone-100">{deck.name}</p>
                  <p className="text-sm text-stone-600 dark:text-stone-400">{deck.description}</p>
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${deck.isPublic ? 'bg-brand-100 text-brand-800 dark:bg-brand-900/50 dark:text-brand-300' : 'bg-stone-200 text-stone-800 dark:bg-stone-600 dark:text-stone-200'}`}>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`p-2 rounded-md transition-colors ${
                      canModifyDeck(deck)
                        ? 'text-brand-600 dark:text-brand-400 hover:bg-brand-50 dark:hover:bg-stone-700 hover:text-brand-800 dark:hover:text-brand-300'
